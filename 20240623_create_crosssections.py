...
def matrix_to_image(matrix):
    ...
    scaled_image = ((image - np.min(image)) / (np.max(image) - np.min(image) + 1) * 255).astype(np.uint8)
    return scaled_image
...
def create_4cs(v_scaling, v_rot, v_shearing, grid_size, unitcell, miller, t):
    ...
    # scaling
    M1 = create_affine_matrix(angles = (0, 0, 0), translation = (0, 0, 0), scaling = v_scaling[0], shearing = (0, 0, 0, 0, 0, 0))
    # rotation
    M2 = create_affine_matrix(angles = v_rot[0], translation = (0, 0, 0), scaling = (1, 1, 1), shearing = (0, 0, 0, 0, 0, 0))
    # shearing
    M3 = create_affine_matrix(angles = (0, 0, 0), translation = (0, 0, 0), scaling = (1, 1, 1), shearing = v_shearing[0])
...
def big_loop(sf, sd, gs, uc, miller, co, square_shift, iv_inp):
    ...
    for best4 in range(0, starting_points):
        w = 0
        t = iv_inp[0][best4]
        v_translations = (0, 0, 0)
        v_scaling = [iv_inp[1][best4][0],
                     iv_inp[1][best4][1],
                     iv_inp[1][best4][2]]
        v_rotations = [iv_inp[2][best4][0],
                       iv_inp[2][best4][1],
                       iv_inp[2][best4][2]]
        v_shearing = [iv_inp[3][best4][0],
                      iv_inp[3][best4][1],
                      iv_inp[3][best4][2],
                      iv_inp[3][best4][3],
                      iv_inp[3][best4][4],
        ...
        top_transf_matrices, top_score, max_location, model_matrices, top_v_sca, top_v_rot, top_v_she, topsurfaces, t, cropped_samples, top_corr_a = perform_crosscorrelation_left(
            ...
            top_v_rotations[best4], top_v_translations[best4], top_v_scaling[best4], top_v_shearing[best4], top_label[best4], top_corr_arr[best4],
            top_v_threshold[best4],
            v_rotations, v_translations, v_scaling, v_shearing)

        start_label[best4] = topsurfaces[1]
        ...
        #         for z in range(0, 360, 10):
                    if v_corr > max(top_corr_arr[best4]):
                        t = iv_inp[0][best4]
                        v_translations = (0, 0, 0)
                        v_scaling = [iv_inp[1][best4][0],
                                     iv_inp[1][best4][1],
                        ...
                        top_transf_matrices, top_score, max_location, model_matrices, top_v_sca, top_v_rot, top_v_she, topsurfaces, t, cropped_samples, top_corr_a = perform_crosscorrelation_left(
                            ...
                            top_L90_scores, top_matrix_L90, max_location_L90, model_matrices_L90,
                            top_L24_scores, top_matrix_L24, max_location_L24, model_matrices_L24,
                            top_v_rotations[best4], top_v_translations[best4], top_v_scaling[best4], top_v_shearing[best4], top_label[best4], top_corr_arr[best4], top_v_threshold[best4],
                            v_rotations, v_translations, v_scaling, v_shearing)

                    if v_corr == max(top_corr_arr[best4]):
                        t = iv_inp[0][best4]
                        #t = iv_inp[0][best4] + pow(-1, random.randint(10)) * wobble_threshold * float(random.randint(100)) / 100
                        v_translations = (0, 0, 0)
                        #vsc_x = iv_inp[1][best4][0] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_sca
                        #vsc_y = iv_inp[1][best4][1] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_sca
                        #vsc_z = 1/(vsc_x * vsc_y)
                        #v_scaling = [vsc_x, vsc_y, vsc_z]
                        v_scaling = [1, 1, 1]
                        v_rotations = [iv_inp[2][best4][0] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_rot,
                                       iv_inp[2][best4][1] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_rot,
                                       iv_inp[2][best4][2] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_rot]
                        #v_shearing = [iv_inp[3][best4][0] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              iv_inp[3][best4][1] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              iv_inp[3][best4][2] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              iv_inp[3][best4][3] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              iv_inp[3][best4][4] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              iv_inp[3][best4][5] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she]
                    ...
                    elif v_corr < max(top_corr_arr[best4]):
                        max_ind = top_corr_arr[best4].index(max(top_corr_arr[best4]))
                        t = iv_inp[0][best4]
                        #t = top_v_threshold[best4][max_ind] + pow(-1, random.randint(10)) * wobble_threshold * float(random.randint(100)) / 100
                        ...
                        #v_scaling = [top_v_scaling[best4][max_ind][0] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_sca,
                        #             top_v_scaling[best4][max_ind][1] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_sca,
                        #             top_v_scaling[best4][max_ind][2] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_sca]
                        v_rotations = [top_v_rotations[best4][max_ind][0] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_rot,
                                       top_v_rotations[best4][max_ind][1] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_rot,
                                       top_v_rotations[best4][max_ind][2] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_rot]
                        #v_shearing = [top_v_shearing[best4][max_ind][0] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              top_v_shearing[best4][max_ind][1] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              top_v_shearing[best4][max_ind][2] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              top_v_shearing[best4][max_ind][3] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                        #              top_v_shearing[best4][max_ind][4] + pow(-1, random.randint(10)) * np.random.randint(0, 100)/100 * wobble_she,
                    ...
                    top_corr_arr[best4] = top_corr_a
                    top_v_rotations[best4] = top_v_rot
                    top_v_scaling[best4] = top_v_sca
                    top_v_shearing[best4] = top_v_she
                    top_v_threshold[best4] = t
                    top_v_surface[best4] = topsurfaces
                    top_v_singlecorr[best4] = top_score
                    top_v_max_location[best4] = max_location
                    top_v_model_matrices[best4] = model_matrices
                    top_v_cropped_samples[best4] = cropped_samples
                    ...
                    save_data_csv(top_v_scaling[best4], top_v_rotations[best4], top_v_shearing[best4], top_v_surface[best4], top_v_singlecorr[best4], top_corr_arr[best4], iterations, time_sofar_used, sf, co, best4, start_label[best4], top_v_threshold[best4])
                    # print(f'corr is: {top_corr_arr}')
                    w = w + 1
                    # print(f'xyz is {v_rotations}')

                    percentage = (float(best4)*float(iterations) + float(round(w))) / (float(starting_points)*float(iterations)) * 100
                    print(f'{round(percentage, 2)}% with {len(top_corr_arr[best4])} of {best_combos} saved iterations')
                    print(f'loop {best4+1} of {starting_points}')
        ...
        for i in range(0, cs_per_volume):
            ...
            plot_4cs(top_v_model_matrices[best4], labels, sf, best4, top_corr_arr[best4], i)


        if co == 'L':
            print_top_correlations_left(max_color, min_color, color_palette, top_v_singlecorr[best4], top_corr_arr[best4],
                                        top_v_model_matrices[best4], top_v_cropped_samples[best4],
                                        top_v_scaling[best4], top_v_rotations[best4], top_v_shearing[best4], top_v_surface[best4], top_v_threshold[best4],
                                        transparency_overlay, transparency_sample, top_v_max_location[best4], sf, co, best4)
        if co == 'R':
            print_top_correlations_right(max_color, min_color, color_palette, top_v_singlecorr[best4], top_corr_arr[best4],
                                         top_v_model_matrices[best4], top_v_cropped_samples[best4],
                                        top_v_scaling[best4], top_v_rotations[best4], top_v_shearing[best4], top_v_surface[best4], top_v_threshold[best4],
                                        transparency_overlay, transparency_sample, top_v_max_location[best4], sf, co, best4)
...
def simply_plot_4cs(bin_im, label_images, directory, k):
    bin_im_np_orig = [0, 0, 0, 0]

    bin_im_np_orig[0] = bin_im[0]
    # plt.imshow(bin_im_np_orig[0])
    cv2.imwrite(directory + f'/{label_images[0]}_crosssection_0.png', bin_im_np_orig[0])

    bin_im_np_orig[1] = bin_im[1]
    # plt.imshow(bin_im_np_orig[1])
    cv2.imwrite(directory + f'/{label_images[1]}_crosssection_1.png', bin_im_np_orig[1])

    bin_im_np_orig[2] = bin_im[2]
    # plt.imshow(bin_im_np_orig[2])
    cv2.imwrite(directory + f'/{label_images[2]}_crosssection_2.png', bin_im_np_orig[2])

    bin_im_np_orig[3] = bin_im[3]
    # plt.imshow(bin_im_np_orig[2])
    cv2.imwrite(directory + f'/{label_images[3]}_crosssection_3.png', bin_im_np_orig[3])


def simply_save_data_csv(tops, topr, topsh, surface_on_top, save, thr):
    # Specify the file path
    gmt = time.gmtime()
    file_path = save + f'/{gmt[0]}{gmt[1]:02}{gmt[2]:02}_crosssections_array_data.csv'
...
def create_crosssections(sf, dir, gs, uc, miller, corner, sample_sh, iv_inp):
    t = iv_inp[0][0]
    print(f'threshold: {iv_inp[0][0]}')
    v_translations = (0, 0, 0)
    print(f'scaling: {iv_inp[1][0]}')
    v_scaling = [iv_inp[1][0]]
    print(f'rotation: {iv_inp[2][0]}')
    v_rotations = [iv_inp[2][0]]
    print(f'shearing: {iv_inp[3][0]}')
    v_shearing = [iv_inp[3][0]]
    ...
    for i in range(0, 1):
        simply_plot_4cs(matrices_binary, labels, sf, i)

    simply_save_data_csv(v_scaling, v_rotations, v_shearing, topsurface, sf, t)
...
saving_files = ["C:/Users/iselire/Downloads/Neuer Ordner"]

# samples should be named sample_F24 etc. or F24 etc.
samples_dir = ["C:/Users/IseliRe/PycharmProjects/pythonProject/20240509_multiprocessing/distorted_test_L",
               "C:/Users/IseliRe/PycharmProjects/pythonProject/20240509_multiprocessing/distorted_test_R"]

initial_values = ["C:/Users/iselire/Downloads/Neuer Ordner/20240713_crosssections_array_data.csv"]
